...
- `RatFunc.valuation_isEquiv_infty_or_adic_of_fintype`: Ostrowski's theorem for `K(X)`, where `K`
is a finite field.
-/
...
theorem uniformizingPolynomial_isUniformizer [vDisc : IsRankOneDiscrete v] :
  ...
  | f p q hne0 =>
    ...
    rw [valuation_eq_valuation_πᵥ_pow_of_valuation_X_le_one hle p hp,
      valuation_eq_valuation_πᵥ_pow_of_valuation_X_le_one hle q hne0] at hx
    ...
    clear hn hm
    have hpi0 : v ↑πᵥ ≠ 0 := by simpa using uniformizingPolynomial_ne_zero hle
...
theorem valuation_isEquiv_valuationIdeal_adic_of_valuation_X_le_one [IsRankOneDiscrete v] :
  ...
  · induction f using RatFunc.induction_on with
    | f p q hq0 =>
      ...
      rw [valuation_eq_valuation_πᵥ_pow_of_valuation_X_le_one hle p hp0,
        valuation_eq_valuation_πᵥ_pow_of_valuation_X_le_one hle q hq0]
...
theorem adicValuation_not_isEquiv_infty_valuation [DecidableEq (RatFunc K)]
  ...
  have hlt : inftyValuation K (1/X) < 1 := by
    rw [one_div, map_inv₀, inftyValuation.X, inv_lt_one_iff₀]
    simp [← exp_zero]
  have hge : 1 ≤ (p.valuation (RatFunc K)) (1/X) := by
...
/-- Ostrowski's Theorem for `K(t)` with `K` any field. -/
theorem valuation_isEquiv_infty_or_adic [DecidableEq (RatFunc K)] :
  ...
  · right; push_neg at hlt
    obtain ⟨pw, hw⟩ := valuation_isEquiv_adic_of_valuation_X_le_one hlt
...
variable (v) in
/-- Ostrowski's Theorem for `K(t)` with `K` a finite field. -/
theorem valuation_isEquiv_infty_or_adic_of_fintype [DecidableEq (RatFunc K)] :
    Xor' (v.IsEquiv (FunctionField.inftyValuation K))
      (∃! (u : HeightOneSpectrum K[X]), v.IsEquiv (u.valuation _)) :=
